...
/* home.html dosyasýndaki bazý verileri burda kullanmak için 
idsi ile elemanlarý deðiþkenlere atama */
            ...
            vote_average
                ...
                <div class="overview">
                    <h3> Overview </h3>
                     ...
                     <button class="favorite" id="${id}"> Favorite </button>
                </div>
            `;
...
//Filmleri favoriye ekleme bölümü
function addToFavorites(title) {
    const userId = loggedInUserId; // Giriþ yapmýþ kullanýcýnýn IDsi alýnýr

    const favoriteMovie = {
        FavoriteFilm: title,
    };
    function addToFavorites(title) {
        // Favori butona týklandýðýnda yapýlacak iþlemler buraya gelecek

        // Mesajý ekrana yazdýrma iþlemi
        console.log("Favorite button clicked for movie: " + title);
    }

    // AJAX isteði gönderin
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "/Home/AddToFavorites");
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function () {
        if (xhr.status === 200) {
            console.log("Favori film veritabanýna eklendi.");
        } else {
...
/* Youtube linklerinin bulunduðu sayfayý açma */
...
/* Youtube navbarý kapatma */
function closeNav() {
    document.getElementById("myNav").style.width = "0%";
}

/* Rating deðerleri renklendirme */
...
/* girilen ifadeye göre film arama iþlemi */
